---
name: "Regulador de Respiración"
category: regulation
audience: [clinician, family]
useContext: [in-session, practice, home]
evidence: clinical-consensus
shortDescription: "Regulador animado de respiración con patrones de respiración cuadrada, 4-7-8 y coherente. Para el cierre, el enraizamiento y el uso en casa."
componentName: "BreathPacer"
citations:
  - label: "Elliott, S. (2008). Coherent Breathing: The Definitive Method — Theory and Practice. COHERENCE."
  - label: "Weil, A. (2000). Breathing: The Master Key to Self Healing. Sounds True. (Técnica 4-7-8 popularizada a través de este programa de audio y drweil.com; sin ensayo clínico aleatorizado independiente.)"
  - label: "Respiración cuadrada (táctica): ampliamente enseñada en entornos militares y de manejo del estrés clínico. Evidencia de respiración lenta: Zaccaro et al. (2018). How Breath-Control Can Change Your Life. Frontiers in Human Neuroscience, 12, 353. Sin una cita única y autoritativa de los Navy SEAL."
warnings: []
locale: es
---

## Qué es esto

Un regulador visual animado para tres patrones de respiración de uso habitual:

- **Respiración cuadrada** (4-4-4-4): inhala 4, sostén 4, exhala 4, sostén 4. Usada en contextos militares y clínicamente para la regulación de la ansiedad aguda.
- **4-7-8**: inhala 4, sostén 7, exhala 8. Enseñanza de Andrew Weil, basada en tradiciones de pranayama.
- **Respiración coherente** (5-5): inhala 5, exhala 5. Asociada con la coherencia de la VFC y la activación parasimpática.

## Cuándo usarla

- **Cierre** (Fase 7) — ayuda a los consultantes a volver a su línea de base.
- **Entre series** si un consultante está muy activado.
- **Uso en casa** — una herramienta de calma que cabe en el bolsillo.

## Notas clínicas

El círculo se expande durante la inhalación y se contrae durante la exhalación. Las pausas detienen la animación. Los preajustes de duración (1, 3, 5 y 10 minutos) ofrecen un punto de parada natural para que el usuario no tenga que mirar el reloj.


La investigación sobre respiración lenta no valida cada patrón específico ni esta herramienta. La comodidad importa más que seguir el reloj: puedes omitir las pausas, respirar normalmente o parar. No se ha establecido que estas duraciones sean adecuadas para todos los niños.
